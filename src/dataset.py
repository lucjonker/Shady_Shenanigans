...
TARGET_H, TARGET_W = 1892, 1903

# Center crop a 2D numpy array to (out_h, out_w).
# Todo: can I use this as a transform in the dataset? Maybe need to make it rasterio based?
...
class DSMShadeDataset(Dataset):
    def __init__(self, index_csv, tile_size=None, transforms=None):
        """
    ...
    def __getitem__(self, idx):
        ...
        dsm_arr = self._read_band(row["dsm"])  # (1992, 2003)
        shade_arr = self._read_band(row["shade_map"])  # (1892, 1903)
